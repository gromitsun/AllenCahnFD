...

{
    ...
    CHECK_RETURN_N(_kernel_3d, clCreateKernel(program, "calc_3d", &err), err);
    ...
    CHECK_ERROR(clSetKernelArg(_kernel_3d, 0, sizeof(cl_mem), &_img_Phi))
    CHECK_ERROR(clSetKernelArg(_kernel_3d, 1, sizeof(cl_mem), &_img_PhiNext))
    CHECK_ERROR(clSetKernelArg(_kernel_3d, 2, sizeof(T), &_M))
    CHECK_ERROR(clSetKernelArg(_kernel_3d, 3, sizeof(T), &_a_2))
    CHECK_ERROR(clSetKernelArg(_kernel_3d, 4, sizeof(T), &_a_4))
    CHECK_ERROR(clSetKernelArg(_kernel_3d, 5, sizeof(T), &_K))
    CHECK_ERROR(clSetKernelArg(_kernel_3d, 6, sizeof(T), &_dx))
    CHECK_ERROR(clSetKernelArg(_kernel_3d, 7, sizeof(T), &_dt))
...
void Simulator_3D<T>::steps(const T & dt, const unsigned int & nsteps, const bool finish, const bool cputime)
{
    ...
    {
        _dt = dt;
        CHECK_ERROR_EXIT(clSetKernelArg(_kernel_3d, 7, sizeof(T), &_dt))
    }
    ...
    for (unsigned int t=0; t<nsteps; t++)
    {
        ...
        CHECK_ERROR_EXIT(clEnqueueNDRangeKernel(Simulator<T>::queue, _kernel_3d, 3, NULL, _global_size, _local_size, 0, NULL, NULL));
        ...
        CHECK_ERROR_EXIT(clSetKernelArg(_kernel_3d, 0, sizeof(cl_mem), &_img_Phi));
        CHECK_ERROR_EXIT(clSetKernelArg(_kernel_3d, 1, sizeof(cl_mem), &_img_PhiNext));
